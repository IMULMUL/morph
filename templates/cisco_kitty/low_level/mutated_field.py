...
from .container import OneOf
from .encoder import ENC_BITS_DEFAULT, ENC_BITS_BYTE_ALIGNED, BitsEncoder
...
class BitFlip(BaseField):
    ...
    def __init__(self, value, num_bits=1, fuzzable=True, name=None):
        ...
        :raises: ``Exception`` if num_bits is bigger than the value length in bits
        :raises: ``Exception`` if num_bits is not positive
        '''
        kassert.is_of_types(value, bytes)
        if len(value) * 8 < num_bits:
            raise Exception('len of value in bits(%d) < num_bits(%d)' % (len(value) * 8, num_bits))
        if num_bits <= 0:
            raise Exception('num_bits(%d) <= 0' % (num_bits))
...
class ByteFlip(BaseField):
    ...
    def __init__(self, value, num_bytes=1, fuzzable=True, name=None):
        ...
        :raises: ``Exception`` if num_bytes is bigger than the value length
        :raises: ``Exception`` if num_bytes is not positive
        '''
        kassert.is_of_types(value, bytes)
        if len(value) < num_bytes:
            raise Exception('len(value) <= num_bytes', (len(value), num_bytes))
        if num_bytes <= 0:
            raise Exception('num_bytes(%d) <= 0' % (num_bytes))
...
class BlockOperation(BaseField):
    ...
    def __init__(self, value, block_size, fuzzable=True, name=None):
        ...
        :raises: ``Exception`` if block_size is bigger than the value length in bytes
        :raises: ``Exception`` if block_size is not positive
        '''
        if block_size > len(value):
            raise Exception('block_size (%d) > length of value (%d)' % (block_size, len(value)))
        if block_size <= 0:
            raise Exception('block_size (%d) <= 0' % (block_size))
...
class BlockDuplicate(BlockOperation):
    ...
    def __init__(self, value, block_size, num_dups=2, fuzzable=True, name=None):
        ...
        '''
        if num_dups <= 0:
            raise Exception('num_dups(%d) <= 0' % (num_dups))
