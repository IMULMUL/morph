...
from .encoder import ENC_STR_DEFAULT, StrEncoder
from .encoder import ENC_INT_DEFAULT, BitFieldEncoder
from .encoder import ENC_BITS_DEFAULT, BitsEncoder
from .encoder import ENC_FLT_DEFAULT, FloatEncoder

from .kitty_object import khash, kassert, KittyObject
...
class BaseField(KittyObject):
    ...
    def __init__(self, value, encoder=ENC_BITS_DEFAULT, fuzzable=True, name=None):
        ...
        '''
        if name and '/' in name:
            raise Exception('Name (%s) includes invalid chars /' % (name))
    ...
    def resolve_field(self, field):
        ...
        :raises: Exception if field could not be resolved
        '''
        ...
        if not resolved_field:
            raise Exception('Could not resolve field by name (%s)' % name)
    ...
    def get_field_by_name(self, name):
        ...
        :raises: :class:`~kitty.core.Exception` if no direct subfield with this name
        '''
        raise Exception('Basic field (%s) does not contain any fields inside (looked for "%s")' % (self, name))
...
class _MultiListAccessor(object):
    ...
    def get(self, idx):
        if (idx < 0) or (idx >= self.size()):
            raise Exception('index out of range: %d list length: %d' % (idx, self.size()))
        old_idx = -1
        while old_idx != idx:
...
def _calc_bitfield_bounds(self, value, minv, maxv):
        if self._length <= 0:
            raise Exception('length (%d) <= 0' % (self._length))
        ...
        if maxv is not None:
            if maxv > self._max_value:
                raise Exception('max_value is too big %d > %d' % (maxv, self._max_value))
            self._max_value = maxv
        if minv is not None:
            if minv < self._min_value:
                raise Exception('min_value is too small %d < %d' % (minv, self._min_value))
            self._min_value = minv
        if self._min_value > self._max_value:
            raise Exception('min_value (%d) > max_value (%d)' % (self._min_value, self._max_value))
        if (value < self._min_value) or (value > self._max_value):
            raise Exception('default value (%d) not in range (min=%d, max=%d)' % (value, self._min_value, self._max_value))
...
class RandomBits(BaseField):
    ...
    def __init__(
        ...
        if unused_bits not in range(8):
            raise Exception('unused bits (%d) is not between 0-7' % unused_bits)
    ...
    def _validate_lengths(self, min_length, max_length):
        ...
        if min_length > max_length:
            raise Exception('min_length(%d) > max_length(%d)' % (min_length, max_length))
        elif min_length < 0:
            raise Exception('min_length(%d) < 0' % (min_length))
        elif max_length <= 0:
            raise Exception('max_length(%d) < 0' % (max_length))
