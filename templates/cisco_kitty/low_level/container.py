...
import random
from .field import BaseField, empty_bits, Dynamic, BitField
from .encoder import BitsEncoder, ENC_BITS_DEFAULT, ENC_BITS_BYTE_ALIGNED
...
class Container(BaseField):
            ...
            for i in range(render_count):
                ...
                for field in self._fields:
                    ...
                    if not isinstance(frendered, Bits):
                        raise Exception('the field %s:%s was rendered to type %s, you should probably wrap it with appropriate encoder' % (
    ...
    def get_field_by_name(self, name):
        ...
        raise Exception('field named (%s) was not found in (%s)' % (self, name))
    ...
    def pop(self):
        ...
        if not self._containers:
            raise Exception('no container to pop')
    ...
    def push(self, field):
        ...
        if container:
            ...
            name = field.get_name()
            if name in self._fields_dict:
                raise Exception('field with the name (%s) already exists in this container' % (name))
            if name:
...
class Repeat(Container):
    ...
    def _check_times(self, min_times, max_times, step):
        ...
        if not((min_times >= 0) and (max_times > 0) and (max_times >= min_times) and (step > 0)):
            raise Exception('one of the checks failed: min_times(%d)>=0, max_times(%d)>0, max_times>=min_times, step > 0' % (min_times, max_times))
...
class Switch(OneOf):
    ...
    def __init__(self, field_dict, key_field, default_key, encoder=ENC_BITS_DEFAULT, fuzzable=True, name=None):
        ...
        if default_key not in field_dict:
            raise Exception('default_key(%s) is not a key in field_dict' % (default_key))
...
class Template(Container):
    ...
    def copy(self):
        ...
        :raises: :class:`~kitty.core.Exception`, as it should not be copied
        '''
        raise Exception('Template should NOT be copied')
