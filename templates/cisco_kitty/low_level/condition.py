...
class FieldCondition(Condition):
    ...
    def invalidate(self, container):
        ...
        if field_name is None:
            raise Exception('Cannot invalidate field without name')
    ...
    def _get_ready(self, container):
        if not self._field:
            if self._field_name:
                ...
                if not field:
                    raise Exception('failed to resolve field name %s' % self._field_name)
                self._field = field
        if not self._field:
            raise Exception('No field provided to base the condition on')
    ...
    def __init__(self, field, comp_type, comp_value):
        ...
        if comp_type in Compare._comparison_types:
            if comp_type not in ['==', '!='] and isinstance(comp_value, str):
                raise Exception('can\'t use comparison type "%s" with comparison value of type str' % comp_type)
            self._comp_fn = Compare._comparison_types[comp_type]
        else:
            raise Exception('unknown comparison type (%s)' % (comp_type))
