...
from .encoder import BitsEncoder
from .encoder import StrEncoder
from .encoder import ENC_BITS_DEFAULT
from .encoder import ENC_STR_DEFAULT
from .encoder import ENC_INT_DEFAULT
from .ll_utils import RenderContext
...
class Calculated(BaseField):
    ...
    def __init__(self, depends_on, encoder=ENC_BITS_DEFAULT, fuzzable=True, name=None):
        ...
        :type encoder: :class:`~.encoder.BitsEncoder`
        ...
        if isinstance(depends_on, bytes):
            ...
            raise Exception('depends_on parameter (%s) is neither a string nor a valid field' % depends_on)
    ...
    def _initialize(self):
        ...
        if not self._field:
            raise Exception('Could not resolve field name %s' % self._field_name)
...
class CalculatedBits(Calculated):
    ...
    def __init__(self, depends_on, func, encoder=ENC_BITS_DEFAULT, fuzzable=True, name=None):
        ...
        try:
            ...
            raise Exception('func should be func(Bits)->Bits')
...
class CalculatedStr(Calculated):
    ...
    def __init__(self, depends_on, func, encoder=ENC_STR_DEFAULT, fuzzable=False, name=None):
        ...
        :type encoder: :class:`~.encoder.StrEncoder`
        ...
        try:
            ...
            raise Exception('func should be func(str)->str')
    ...
    def _render(self):
        if len(self._rendered_field) % 8 != 0:
            raise Exception('Hashed data should be byte aligned')
    ...
    def __init__(self, depends_on, algorithm, encoder=ENC_STR_DEFAULT, fuzzable=False, name=None):
        ...
        if algorithm in Hash._algos:
            ...
            try:
                ...
                raise Exception('algorithm should be a func(str)->str or one of the strings %s' % (Hash._algos.keys(),))
...
class FieldIntProperty(CalculatedInt):
    ...
    The main difference from :class:`~.calculated.CalculatedInt`
    ...
    def __init__(self, depends_on, length, correction=None, encoder=ENC_INT_DEFAULT, fuzzable=False, name=None):
        ...
        :type encoder: :class:`~.encoder.BitFieldEncoder`
        ...
        if correction:
            if not callable(correction):
                if not isinstance(correction, int):
                    raise Exception('correction must be int, function or None!')
    ...
    def __init__(self, depends_on, length, algorithm='crc32', encoder=ENC_INT_DEFAULT, fuzzable=False, name=None):
        ...
        if algorithm in Checksum._algos:
            ...
            try:
                ...
                raise Exception('algorithm should be a func(str)->int or one of the strings %s' % (Checksum._algos.keys(),))
...
class Size(CalculatedInt):
    ...
    .. note::
        ...
        In most cases you can use the function :func:`~.aliases.SizeInBytes`
...
class Offset(FieldIntProperty):
    ...
    def __init__(self, base_field, target_field, length, correction=None, encoder=ENC_INT_DEFAULT, fuzzable=True, name=None):
        ...
        :examples:
            ...
            (also, see :class:`~.calculated.AbsoluteOffset`)
        ...
        if isinstance(base_field, bytes):
